...
- `DetailsScreen` usa `LayoutBuilder` para adaptar o espaçamento lateral conforme a largura disponível.
...
- Em telas pequenas, o ocupa o espaço disponível, respeitando as margens laterais.
- O botão fica alinhado à esquerda do conteúdo e possui largura máxima de `300 px`, evitando ficar excessivamente grande em monitores largos.
- O `InfoCard` ocupa toda a largura do conteúdo limitado.
- Cada linha do `InfoCard` possui explicitamente `width: double.infinity`, fazendo as linhas divisórias percorrerem toda a largura do card.
- O texto dos rótulos e valores foi centralizado dentro de cada linha, sem alterar a largura das linhas divisórias.
- Foi utilizado `SingleChildScrollView` para evitar overflow quando a altura disponível for pequena, como em janelas baixas ou em modo paisagem.
- O layout usa `Column`, `Align`, `ConstrainedBox`, `Padding` e `SizedBox`, evitando `Container` apenas para estruturar o layout.
